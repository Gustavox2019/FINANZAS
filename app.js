...
/* ------------------------------------------------------------------ */
/*  React / Recharts (cargados como globales vía CDN en index.html)    */
/* ------------------------------------------------------------------ */
          ...
          {gastosPorCategoria.length === 0 ? (
            ...
            <>
              <ResponsiveContainer width="100%" height={200}>
                <PieChart>
                  <Pie data={gastosPorCategoria} dataKey="value" nameKey="name" innerRadius={50} outerRadius={80} paddingAngle={2}>
                    {gastosPorCategoria.map((entry, i) => (
                      <Cell key={entry.name} fill={GRUPO_COLORS[entry.name] || PALETTE[i % PALETTE.length]} />
                    ))}
                  </Pie>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        <div className="rounded-2xl border border-stone-200 bg-white p-5 shadow-sm">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={evolucion6m.map((d) => ({ mes: d.mes, Ahorro: Number((d.Ingresos - d.Gastos).toFixed(2)) }))}>
              <CartesianGrid strokeDasharray="3 3" stroke="#e7e5e4" />
              <XAxis dataKey="mes" tick={{ fontSize: 12 }} />
              <YAxis tick={{ fontSize: 12 }} />
              <Tooltip formatter={(v) => fmt(v)} />
              <Line type="monotone" dataKey="Ahorro" stroke="#0d9488" strokeWidth={2.5} dot={{ r: 3 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
